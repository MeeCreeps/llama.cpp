...
llm_build_gemma4_iswa::llm_build_gemma4_iswa(const llama_model & model, const llm_graph_params & params) :
    ...
    if (model.tok_embd_per_layer) {
        inp_per_layer = get_per_layer_inputs();
    ...
    for (int il = 0; il < n_layer; ++il) {
        const int64_t n_embd_head = hparams.is_swa(il) && hparams.n_embd_head_k_swa ? hparams.n_embd_head_k_swa : hparams.n_embd_head_k;
        const int64_t n_embd_head_v = hparams.is_swa(il) && hparams.n_embd_head_v_swa ? hparams.n_embd_head_v_swa : hparams.n_embd_head_v;
        ...
        const int   n_rot_l      = std::min((int) hparams.n_rot, (int) n_embd_head);
