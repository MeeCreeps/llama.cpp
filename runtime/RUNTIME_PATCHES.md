...
| H2 | `ggml/src/ggml-opencl/ggml-opencl.cpp` | `buffer_context` + `alloc_buffer` + `init_tensor` + `buffer_clear` | **ε 方案 per-tensor，懒决策**：alloc_buffer 时拿不到 tensor 信息，进 PENDING 态；首个 init_tensor 看 tensor 名 `.weight` 后缀 → 升 ELASTIC（per-tensor cl_mem）；否则 → 升 MONOLITHIC（lazy 把整块 cl_mem 真分配出来）。KV cache 与 compute buffer 走 MONOLITHIC、weight buffer 走 ELASTIC。buffer_clear 在非 MONOLITHIC 模式 no-op。`GGML_OPENCL_ELASTIC=1` env 守护。 | ✅ DONE 2026-05-14 |
| H2.5 | `src/llama-model-loader.cpp` (≈L1063) | `load_all_data` 的 OpenCL 路径 | 上游 bug：非-host backend (OpenCL/CUDA 等) 走 `ggml_backend_tensor_set` 时不更新 `mmaps_used`，导致 load 结束 `unmap_fragment` 把整个 mmap 拆掉。F4-evict 时存的 host_ptr 失效 → segfault。在 else 分支同步更新 mmaps_used 让 unmap 只拆未用区域，weight 区域保留。 | ✅ DONE 2026-05-14 |
| H2.6 | `src/llama-model.cpp` + `ggml/src/ggml-rpc/ggml-rpc.cpp` | 调 `get_base` 的位置 | get_base 已经返回 alignment 假指针，原本就和 ELASTIC 没本质冲突。mlock 与 ggml-rpc 兼容性留 TODO。 | TODO |
| H4 | `ggml/src/ggml-opencl/ggml-opencl.cpp` | `ggml_backend_opencl_graph_compute` | **ε 方案 per-tensor**：遍历 cgraph 节点，每个 src tensor 是 WBM 注册过的 weight → `wbmcl_ensure_resident(tensor_id)` + `wbm_touch`；dispatch op 之后每 EVICT_CHECK_INTERVAL 个 op 调一次 `wbm_evict_to_byte_budget` + `wbmcl_evict_batch`，**字节预算 + 批量 flush**。clFinish 保证 in-flight kernel 不会读 just-released cl_mem (F4-event 后改 cl_event 精细同步)。fused 分支额外 ensure_resident 被融合进来的下一个节点的 srcs。 | ✅ DONE 2026-05-14 |
