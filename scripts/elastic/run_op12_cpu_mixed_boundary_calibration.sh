...
base_profile="${ELASTIC_BASE_GRANULARITY_PROFILE:-runtime/plan/profiles/granularity/op12_llama3_8b_q4_cpu_pipeline_budgetfair_i8mm_20260728.json}"
cost_dir="${ELASTIC_COST_DIR:-runtime/plan/profiles/android-cpu/Meta-Llama-3-8B-Instruct-Q4_0_op12_i8mm_20260728}"
calibration_root="${ELASTIC_CALIBRATION_ROOT:-exp-results/Meta-Llama-3-8B-Instruct-Q4_0/granularity_cpu_mixed_boundary_nodes_op12_20260729}"
selected_profile="${ELASTIC_SELECTED_PROFILE:-runtime/plan/profiles/granularity/op12_llama3_8b_q4_cpu_pipeline_mixedboundary_20260729.json}"
...
for penalty in "${penalty_values[@]}"; do
  ...
  # The pure Tensor reference is independent of the boundary candidate and
  # is measured once. Every candidate then runs the real offline mixed
  # frontier over the same four fixed budget nodes.
