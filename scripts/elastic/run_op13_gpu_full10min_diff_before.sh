...
# Same GPU phase costs and edit rate as Diff-now; only the learned exposed
# pipeline-efficiency residual is absent from this Diff-before profile.
python3 runtime/plan/run_dynamic_budget_matrix.py \
  ...
  --artifact-root exp-results/Meta-Llama-3-8B-Instruct-Q4_0/difftree_gpu_native_full10min_diff_before_phaseonly_op13_20260728_graph1_budgetsafe_v3 \
  --window-sec 600 \
  --window-start-sec 0 \
  --replay-speedup 1 \
  --bench-seconds 600 \
  ...
  --granularity-profile runtime/plan/profiles/granularity/op13_llama3_8b_q4_gpu_pipeline_phase_only_20260727.json \
