...
# Held-out reconfiguration-rate check. This window is used only to verify that
# Diff-tree can follow a descending/rising budget fast enough; the final result
# is always measured on the original 0--600 second trace.
python3 runtime/plan/run_dynamic_budget_matrix.py \
  ...
  --artifact-root exp-results/Meta-Llama-3-8B-Instruct-Q4_0/difftree_cpu_native_nodes80_140_edits16_multi_overhead_smoke_op13_20260727 \
  --window-sec 60 \
  --window-start-sec 80 \
  --replay-speedup 1 \
  --bench-seconds 60 \
